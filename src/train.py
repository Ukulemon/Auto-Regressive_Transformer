...
def train_epoch(
    ...
    - ``scaler``：来自``torch.amp``的缩放器，用于混合精度。
    ...
    for step, (inputs, targets) in enumerate(tqdm(dataloader, desc="Training", leave=False)):
        ...
        autocast_kwargs = {"enabled": mixed_precision and device.type == "cuda"}
        if device.type == "cuda":
            autocast_kwargs.update({"device_type": "cuda", "dtype": torch.float16})
        else:
