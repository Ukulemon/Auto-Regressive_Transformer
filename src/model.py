...
class AutoregressiveTransformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        causal_mask_buffer = self._causal_mask
        if seq_len > causal_mask_buffer.size(0):
            new_size = max(seq_len, causal_mask_buffer.size(0) * 2)
            causal_mask_buffer = torch.triu(
                torch.ones((new_size, new_size), device=device, dtype=torch.bool),
                diagonal=1,
            )
            self.register_buffer("_causal_mask", causal_mask_buffer, persistent=False)
        elif causal_mask_buffer.device != device:
            causal_mask_buffer = causal_mask_buffer.to(device=device)
            self.register_buffer("_causal_mask", causal_mask_buffer, persistent=False)
        causal_mask = causal_mask_buffer[:seq_len, :seq_len]
